...
public:
  ...
  DA2Network( const char *network_path ) {
    ...
    this->session_ = make_session(network_path);
  }
 ...
 private:
  Ort::Session *make_session( const char *network_path ) {
#ifdef _WIN32
    std::wstring wide_path(network_path, network_path + std::strlen(network_path));
    return new Ort::Session(env, wide_path.c_str(), Ort::SessionOptions{nullptr});
#else
    return new Ort::Session(env, network_path, Ort::SessionOptions{nullptr});
#endif
  }
